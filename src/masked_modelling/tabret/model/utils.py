...
def get_diff_columns(
    ...
    continuous_columns = [col for col in continuous_columns if col not in pre_continuous_columns]
    cat_cardinality_dict = {
        col: item for col, item in cat_cardinality_dict.items() if col not in pre_cat_cardinality_dict
    }
