    ...
    def risk_assessment(self, experiment_class: Type[ExperimentRunner]) -> None:
        ...
        for cls in range(self.num_cls):
            ...
            if not os.path.exists(json_results):
                ...
                print(f"File {json_results} already present! Shutting down to prevent loss of previous experiments")
                continue

        self.process_results()

    def _risk_assessment_helper(self, cls: int, experiment_class: Type[ExperimentRunner], exp_path: str) -> None:
        """Run the experiment and process the results."""
        ...
        if best_config["save_scores"]:
            ...
            with open(save_path, "w") as fp:
                json.dump(saved_results, fp, indent=0)
