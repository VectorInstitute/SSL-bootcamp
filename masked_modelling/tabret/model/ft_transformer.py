...
class NumericalFeatureTokenizer(nn.Module):
    ...
    def add_parameters(
        ...
        weight_add, bias_add = self.get_dict_parameter(
            continuous_columns, self.bias is not None
        )
...
class CategoricalFeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.embeddings = nn.ModuleDict(
            {
                key: nn.Embedding(cardinality, d_token)
                for key, cardinality in cardinality_dict.items()
            }
        )
        if bias:
            bias = {}  # type: ignore
    ...
    def add_parameters(self, cardinality_dict: Dict[str, int]) -> None:
        ...
        embeddings_add = nn.ModuleDict(
            {
                key: nn.Embedding(cardinality, self.__d_token)
                for key, cardinality in cardinality_dict.items()
            }
        )
...
class FeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.cat_tokenizer = (
            CategoricalFeatureTokenizer(
                cat_cardinality_dict, d_token, True, self.initialization
            )
    ...
    def forward(
        self, x_num: Optional[Dict[str, Tensor]], x_cat: Optional[Dict[str, Tensor]]
    ) -> Tensor:
        """Perform the forward pass.
...
class Transformer(nn.Module):
    ...
    def _end_residual(
        self, layer: nn.ModuleDict, stage: str, x: Tensor, x_residual: Tensor
    ) -> Tensor:
        assert stage in ["attention", "ffn"], _INTERNAL_ERROR_MESSAGE
