...
class TabRet(nn.Module):
    ...
    def __init__(
        ...
        self.alignment_layer = nn.Linear(
            encoder_embed_dim, encoder_embed_dim, bias=True
        )
        ...
        if continuous_columns is not None:
            self.decoder_pos_embed.update(
                {
                    key: nn.Parameter(Tensor(1, 1, decoder_embed_dim))
        ...
        if cat_cardinality_dict is not None:
            self.decoder_pos_embed.update(
                {
                    key: nn.Parameter(Tensor(1, 1, decoder_embed_dim))
                    for key in cat_cardinality_dict.keys()
                }
            )
        ...
        if continuous_columns is not None:
            self.projectors.update(
                {key: nn.Linear(decoder_embed_dim, 1) for key in continuous_columns}
            )
        if cat_cardinality_dict is not None:
            self.projectors.update(
                nn.ModuleDict(
                    {
                        key: nn.Linear(decoder_embed_dim, out_dim)
                        for key, out_dim in cat_cardinality_dict.items()
                    }
                )
            )
    ...
    def optimization_param_groups(self) -> List[Dict[str, Any]]:
        ...
        non_decay_parameters = get_parameter_names(
            self, [nn.LayerNorm, FeatureTokenizer]
        )
        decay_parameters = [
            name for name in non_decay_parameters if ".bias" not in name
        ]
        return [
            {
                "params": [
    ...
    def freeze_parameters_wo_specific_columns(self, columns: List[str]) -> None:
        ...
        for name, p in self.named_parameters():
            ...
            if len(name_split) > 1 and (
                name_split[-1] in columns or name_split[-2] in columns
            ):
                p.requires_grad = True
                continue
            p.requires_grad = False

    def freeze_transfomer(self) -> None:
    ...
    def add_attribute(
        ...
        if cat_cardinality_dict is not None:
            for key, cardinality in cat_cardinality_dict.items():
                ...
                self.projectors.update(
                    {key: nn.Linear(self.decoder_embed_dim, cardinality)}
                )
    ...
    def custom_masking(
        ...
        ids_restore = torch.ones(N, L, device=x.device) * torch.arange(
            L, device=x.device
        )
        return x_masked, mask, ids_restore.to(torch.int64)

    def column_shuffle(
        self, x: Tensor, column_shuffle_ratio: float, mask: Optional[List[int]] = None
    ) -> Tensor:
        """Shuffle the columns."""
        N, L, _ = x.shape
        ...
        shuffle_idx_shift = torch.cat(
            [ids_shuffle[:, None], torch.randint(N, (L, 1), device=x.device)], dim=1
        )[:num_noise]
    ...
    def forward_encoder(
        ...
        if type(mask_ratio) != list:
            x, mask, ids_restore = self.random_masking(  # type: ignore
                x, mask_ratio, shuffle_idx_shift  # type: ignore
            )
        else:
    ...
    def forward(
        ...
        x, mask, ids_restore, shuffle_idx_shift = self.forward_encoder(
            x_num, x_cat, mask_ratio, col_shuffle  # type: ignore
        )
        preds = self.forward_decoder(
            x, ids_restore, keys, shuffle_idx_shift, col_shuffle
        )
