...
class TabRetClassifier(nn.Module):
    ...
    def __init__(self, pretrained_tabret: TabRet, output_dim: int) -> None:
        ...
        self.pos_embed_target = nn.Parameter(
            Tensor(1, 1, pretrained_tabret.decoder_embed_dim)
        )
        pretrained_tabret.initialization_.apply(
            self.pos_embed_target, pretrained_tabret.decoder_embed_dim
        )
