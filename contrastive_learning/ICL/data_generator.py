...
class DataGenerator:
    ...
    def generate_realistic_synthetic(  # noqa: C901
        ...
        1. local outliers: where normal data follows the GMM distribuion, and
        anomalies follow the GMM distribution with modified covariance
        2. global outliers: where normal data follows the GMM distribuion, and
        ...
        by GaussianKDE
        4. cluster outliers: where normal data follows the GMM distribuion, and
    ...
    def add_label_contamination(
        ...
        if noise_ratio != 0.0:
            ...
            # filpped to another class with probability p (i.e., noise ratio)
    ...
    def generator(  # noqa: C901
        ...
        realistic_synthetic_mode: Optional[
            Literal["local", "cluster", "dependency", "global"]
        ] = None,
        alpha: int = 5,
        ...
        if self.dataset is None:
            ...
            if (
                self.dataset_list_classical is not None
                and self.dataset in self.dataset_list_classical
            ):
                data = np.load(
                ...
                self.dataset_list_cv is not None
                and self.dataset in self.dataset_list_cv
            ):
                data = np.load(
                ...
                self.dataset_list_nlp is not None
                and self.dataset in self.dataset_list_nlp
            ):
                data = np.load(
        ...
        # if the dataset is too small, generating duplicate smaples up to
        # n_samples_threshold
        if len(y) < self.n_samples_threshold and self.generate_duplicates:  # type: ignore # noqa: E501
            ...
            idx_duplicate = np.random.choice(
                np.arange(len(y)), self.n_samples_threshold, replace=True  # type: ignore # noqa: E501
            )
            X = X[idx_duplicate]
            y = y[idx_duplicate]
        ...
        if isinstance(la, float):
            ...
            if la > len(idx_anomaly):
                raise AssertionError(
                    "The number of labeled anomalies are greater than the total "
                    f"anomalies: {len(idx_anomaly)} !"
                )
            else:
