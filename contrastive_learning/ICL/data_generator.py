...
currently, data generator only supports for generating the binary,
...
# currently, data generator only supports for generating the binary classification datasets
class DataGenerator:
    """Data Generator Class  - supports for generating the binary classification datasets.
    ...
    def add_duplicated_anomalies(self, X, y, duplicate_times: int):  # type: ignore
    ...
    def add_irrelevant_features(self, X, y, noise_ratio: float):  # type: ignore
    ...
    def add_label_contamination(self, X, y, noise_ratio: float):  # type: ignore
        if noise_ratio == 0.0:
            pass
        else:
    ...
    def generator(
        ...
        if type(la) == float:
            if at_least_one_labeled:
                ceil(sum(y) * (1 - self.test_size) * la)
            else:
                int(sum(y) * (1 - self.test_size) * la)
        elif type(la) == int:
            pass
        else:
            raise NotImplementedError
