...
class runExperiment:
    def __init__(self, model_configuration, exp_path):  # type: ignore
        self.model_config = Config.from_dict(model_configuration)
        self.exp_path = exp_path

    def run_test(
        self, train_data, val_data, test_data, logger, contamination, query_num
    ):  # type: ignore
        ...
        shuffle = (
            self.model_config["shuffle"] if "shuffle" in self.model_config else True
        )
        ...
        train_loader = DataLoader(
            train_data, batch_size=batch_size, shuffle=shuffle, drop_last=False
        )
        ...
        trainer = trainer_class(
            model,
            loss_function=loss_class(self.model_config["loss_temp"]),
