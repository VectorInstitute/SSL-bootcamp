...
def read_config_file(dict_or_filelike):  # type: ignore
...
class Logger:
    def __init__(self, filepath, mode, lock=None):  # type: ignore
...
def compute_pre_recall_f1(target, score):  # type: ignore
...
class EarlyStopper:
    def stop(
        ...
        raise NotImplementedError("Implement this method!")  # type: ignore

    def get_best_vl_metrics(self):  # type: ignore
        return (
            self.train_loss,
            self.val_loss,
            self.val_auc,
...
class Patience(EarlyStopper):
    ...
    def __init__(self, patience=10, use_train_loss=True):  # type: ignore
