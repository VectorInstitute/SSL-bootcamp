...
def CIFAR10_feat(
    ...
    idx_contamination = np.random.choice(
        np.arange(train_contamination.shape[0]), num_contamination, replace=False
    )
...
def synthetic_contamination(
    ...
    train_contamination = train_contamination + np.random.randn(
        *train_contamination.shape
    ) * np.std(anorm, 0, keepdims=True)
...
def Thyroid_data(
    ...
    train, train_label = synthetic_contamination(
        train_norm, outliers, contamination_rate
    )
...
def load_data(data_name: str, cls: int, contamination_rate: float = 0.0) -> Tuple:
    ...
    if data_name == "cifar10_feat":
        train, train_label, test, test_label = CIFAR10_feat(
            cls, contamination_rate=contamination_rate
        )
    elif data_name == "fmnist_feat":
        train, train_label, test, test_label = FMNIST_feat(
            cls, contamination_rate=contamination_rate
        )
